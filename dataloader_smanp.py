...
def data_load():
    ...
    train_y = train_data[:, (0, 6)]
    valid1_x = valid_data[:, 1:6]
    valid_y = valid_data[:, (0, 6)]
    ...
    valid_x = torch.Tensor(valid1_x).to(device).float()
...
class DatasetGP(Dataset):
    ...
    def __getitem__(self, index):
        ...
        n_target = n_context + np.random.randint(3, 187 - n_context + 1)
