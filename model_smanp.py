...
class CrossEncoder(nn.Module):
    ...
    def forward(self, context_x, context_y, target_x):
        ...
        query = self.target_projection(target_x[..., 2:])  # (bs,nt,12)--> (bs,nt,num_hidden)
        keys = self.context_projection(context_x[..., 2:])  # (bs,nc,12)--> (bs,nc,num_hidden)
...
class SpatialNeuralProcess(nn.Module):
    def __init__(self, x_size, y_size, num_hidden):
        ...
        self.LatenEncoder = LatenEncoder(self.x_size, self.y_size, self.num_hidden)
...
class Criterion(nn.Module):
    ...
    def forward(self, prior_mu, prior_sigma, mu, sigma, target_y):
        ...
        for i in range(bs):
            ...
            loss += -(log_prob - kl_loss)  # torch.mean(log_prob)
            # loss += -log_prob  # torch.mean(log_prob)
        loss = loss / bs
